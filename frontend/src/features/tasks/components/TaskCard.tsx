...
export function TaskCard({
  ...
  return (
    <article className="idea-card task-card">
      <div className={`card-art ${categoryTone(task.category)}`}>
        <span className="art-emoji">
          {task.category === 'Экология' ? '🌱' : task.category === 'Образование' ? '✦' : '◈'}
        </span>
      ...
      <div className="card-content">
        <div className="card-topline">
          ...
          <span className="rating">
            <Star size={14} fill="currentColor" /> {task.rating}/100
          </span>
        </div>
        <h3>{task.title}</h3>
        <p className="card-desc">{task.need || task.context}</p>
        <div className="task-mini-meta">
          <span>{task.author}</span>
          <span>
            <MessageCircle size={13} /> {task.responses.length} откликов
          </span>
        </div>
        <div className="card-bottom">
          <button className="outline-button task-detail-button" onClick={() => onOpen(task)}>
            Подробнее <ArrowUpRight size={16} />
          </button>
          <button
            className="card-arrow"
            onClick={() => onRespond(task)}
            aria-label="Откликнуться на задачу"
          >
